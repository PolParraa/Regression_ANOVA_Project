...
this is quite nice but too large imo, myb we should discuss what else we can delete...
...
We need to align between test data and our model in order to proceed. Test and model need to have the same variables and data types. 


```{r}
library(readr)
test <- read.csv("test.csv")
sample_values <- read.csv("sample_submission.csv")
sample_values$SalePrice <- sample_values$SalePrice / 10000

columns_to_keep_test <- c("Id", "LotArea", "YearBuilt", "YearRemodAdd", "MasVnrArea",
                          "BsmtFinSF1", "BsmtFinSF2", "BsmtUnfSF", "X1stFlrSF", "X2ndFlrSF",
                          "LowQualFinSF", "BsmtFullBath", "BsmtHalfBath", "FullBath", "HalfBath",
                          "BedroomAbvGr", "KitchenAbvGr", "TotRmsAbvGrd", "Fireplaces", "GarageCars",
                          "GarageArea", "WoodDeckSF", "OpenPorchSF", "EnclosedPorch", "X3SsnPorch",
...
test <- test[, columns_to_keep_test, drop = FALSE]
head(test)
head(sample_values)
...
sample_values$Id <- as.character(sample_values$Id)

# Merge the two data frames by Id
sample_test <- merge(test, sample_values, by = "Id")

# View the first few rows of the merged data frame to verify
head(sample_test)
```


*Check for missing values and return $Id of that row*
```{r}
# Check for missing values
missing_rows <- rowSums(is.na(sample_test))
total_missing_rows <- sum(missing_rows > 0)
print(total_missing_rows)  # Output the number of rows with missing values

# Omit missing values
sample_test_clean <- na.omit(sample_test)
print(dim(sample_test))
print(dim(sample_test_clean))
```


*Create aggregations, so it fits the model created before*

```{r}
# Aggregation for MSSubClass
sample_test_clean$MSSubClass <- as.factor(ifelse(sample_test_clean$MSSubClass %in% c("30", "45", "50"), "LowTier", 
                                              ifelse(sample_test_clean$MSSubClass %in% c("60", "75", "120"), "MidTier", "HighTier")))

# Aggregation for Neighborhood
sample_test_clean$Neighborhood <- as.factor(ifelse(sample_test_clean$Neighborhood %in% c("NAmes", "NWAmes", "NPkVill", "Veenker"), "North", 
                                      ifelse(sample_test_clean$Neighborhood %in% c("CollgCr", "Gilbert", "Mitchel"), "Central", 
                                      ifelse(sample_test_clean$Neighborhood %in% c("OldTown", "Edwards", "BrkSide", "BrDale", "IDOTRR", "SWISU"), "East", 
                                      ifelse(sample_test_clean$Neighborhood %in% c("Somerst", "NridgHt", "ClearCr", "StoneBr", "Timber"), "West", "South")))))

# Aggregation for Exterior1st
sample_test_clean$Exterior1st <- as.factor(ifelse(sample_test_clean$Exterior1st %in% c("Wd Sdng", "WdShing"), "Wood", 
                                     ifelse(sample_test_clean$Exterior1st %in% c("VinylSd", "Plywood"), "VinylPly", "Other")))

# Aggregation for OverallQual
sample_test_clean$OverallQual <- as.factor(ifelse(sample_test_clean$OverallQual %in% c("2", "3", "4"), "Low", 
                                     ifelse(sample_test_clean$OverallQual %in% c("5", "6", "7"), "Medium", "High")))

# Aggregation for SaleType
sample_test_clean$SaleType <- as.factor(ifelse(sample_test_clean$SaleType %in% c("WD", "CWD", "VWD"), "Warranty_Deed", 
                                  ifelse(sample_test_clean$SaleType %in% c("Con", "ConLw", "ConLI", "ConLD"), "Contract_Sale", 
                                  ifelse(sample_test_clean$SaleType %in% c("New", "COD", "Oth"), "Other", NA))))
...
*Predict*
```{r}
predictions <- predict(m_full_reduced, sample_test_clean)
...
In theory, the R-squared can return values in the interval (-inf, 1), which can happen when the predictions are too much off from the actual values. That can result into much higher rss than tss, which subsequently results in negative R-squared. This would suggest very weakly fitted model. 

```{r}
# R-squared calculation
actuals <- sample_test_clean$SalePrice   #to be checked if sale price is correct. 
...
*Try for bidirectional model*
```{r}
# prediction using the model
predictions_bi <- predict(m_bi, sample_test_clean)
...
# R-squared calculation
actuals <- sample_test_clean$SalePrice #to be checked if sale price is correct. 
rss <- sum((predictions_bi - actuals)^2)
tss <- sum((actuals - mean(actuals))^2)
r_squared <- 1 - (rss / tss)
...
```{r}
prediction_intervals <- predict(m_full_reduced, sample_test_clean, interval = "prediction", level = 0.95)
head(prediction_intervals)

```


*Checking what percentage of actual values are within the predicted intervals*

```{r}
actuals <- sample_test_clean$SalePrice

# check if actual value is in the interval, gets us binary vector
in_interval <- actuals >= prediction_intervals[, "lwr"] & actuals <= prediction_intervals[, "upr"]

#Calculate the percentage of actual values that fall within the intervals
percentage_within_interval <- mean(in_interval) * 100

print(percentage_within_interval)
```
...
absolute_deviations <- abs(predictions - actuals) 
